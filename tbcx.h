 ...
 *
 *                   v92 method-scope amendment (2026-06-12): each method
 *                   record carries a `scope` u8 immediately AFTER `kind`
 *                   and before `name` (see TBCX_MSCOPE_*).  It records the
 *                   TclOO visibility the method must have after load —
 *                   public (exported), unexported, or TIP 500 true-private
 *                   — as established by the source's method-definition
 *                   options (-export/-private/-unexport), lexical `private`
 *                   context, and same-body export/unexport/self-export/
 *                   self-unexport commands.  Without it a true-private
 *                   method silently downgraded to an inherited ordinary
 *                   method, and a method's explicit export option was lost.
 *                   This is a v92 schema correction, NOT a format bump:
 *                   TBCX_FORMAT stays 92, and every .tbcx artifact must be
 *                   regenerated by a matched saver/loader pair (there are
 *                   no backward-compatibility guarantees for v92 yet). */
#define TBCX_FORMAT 92u

/* Method visibility scope (v92 method record `scope` u8).  Mirrors the
