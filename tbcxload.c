...
static Tcl_Namespace* EnsureNamespaceFromString(Tcl_Interp* interp, const char* nsName, Tcl_Namespace* fallback);
...
static const struct AuxDataType* GetAuxTypeByKind(unsigned char k);
...
static int ReadByteCode(Tcl_Interp* interp,
                        Tcl_Channel ch,
                        Tcl_Namespace* nsPtr,
                        TbcxBCSource src,
                        const TbcxHeader* topHdr,
                        uint32_t formatIfProc,
                        ByteCode** out,
                        uint32_t* outNumLocals);

static int InstallPrecompiledProc(Tcl_Interp* interp,
                                  Tcl_Namespace* targetNs,
                                  const char* fqName,
                                  const char* argSpec,
                                  ByteCode* bc,
                                  uint32_t numLocals,
...
static Tcl_Namespace* EnsureNamespaceFromString(Tcl_Interp* interp, const char* nsName, Tcl_Namespace* fallback)
{
    ...
    TclGetNamespaceForQualName(
        interp, nsName, (Namespace*)Tcl_GetGlobalNamespace(interp), 0, &containerNs, &dummyAlt, &dummyActual, &simple);
    (void)simple;
...
/*
 * GetAuxTypeByKind
 * Map serialized Aux kind tag to AuxDataType pointer used by Tcl.
 */
...
static const struct AuxDataType* GetAuxTypeByKind(unsigned char k)
{
    switch (k)
    {
        ...
        case TBCX_AUX_DICTUPD:
            return tbcxAuxDictUpdate;
        case TBCX_AUX_NEWFORE:
        case TBCX_AUX_FOREACH:
            return tbcxAuxForeach;
        default:
            return NULL;
    }
}
                        ...
                        uint32_t* outNumLocals)
{
    ...
    for (uint32_t i = 0; i < numAux; ++i)
    {
        unsigned char kind;
        R1(kind);
        const struct AuxDataType* tp = GetAuxTypeByKind(kind);
        if (!tp)
            goto bad;
        ...
        else if (kind == TBCX_AUX_NEWFORE || kind == TBCX_AUX_FOREACH)
        {
            ...
            for (uint32_t j = 0; j < lists; ++j)
            {
                ...
                if (nv == 0 || nv > TBCX_MAX_LITERALS) goto bad;
    ...
    for (uint32_t i = 0; i < numEx; ++i)
    {
        ...
        xr[i].codeOffset = (int)le32(sb);
        int start = (int)le32(sb);
        int end = (int)le32(eb);
        xr[i].codeOffset = start;
        if (end < start)
            goto bad;
        xr[i].numCodeBytes = end - start;
        xr[i].continueOffset = (int)le32(cb);
        xr[i].breakOffset = (int)le32(bb);
        xr[i].catchOffset = (int)le32(hb);
        if ((size_t)end > codeLen) goto bad;
        xr[i].numCodeBytes = end - start;
        int cont = (int)le32(cb);
        int brk  = (int)le32(bb);
        int cat  = (int)le32(hb);
        if ((cont != -1 && (cont < 0 || (size_t)cont >= codeLen)) ||
            (brk  != -1 && (brk  < 0 || (size_t)brk  >= codeLen)) ||
            (cat  != -1 && (cat  < 0 || (size_t)cat  >= codeLen))) {
            goto bad;
        }
    ...
    PrewarmLambdaLiterals(interp, codePtr->objArrayPtr, codePtr->numLitObjects);
...
static int LoadFromChannel(Tcl_Interp* interp, Tcl_Channel ch, Tcl_Namespace* nsPtr)
{
    ...
    if (H.codeLen > TBCX_MAX_CODE || H.numLiterals > TBCX_MAX_LITERALS || H.numAux > TBCX_MAX_AUX ||
        H.numExcept > TBCX_MAX_EXCEPT)
    {
    ...
    for (uint32_t i = 0; i < numProcs; ++i)
    {
        ...
        Tcl_Namespace* srcNsPtr = EnsureNamespaceFromString(interp, (nsL ? ns : NULL), nsPtr);
        ...
        if (InstallPrecompiledProc(interp, nsPtr, name, args, pbc, pbcNumLocals, &createdTok) != TCL_OK)
        {
    ...
    for (uint32_t i = 0; i < numMethods; ++i)
    {
        ...
        Tcl_Namespace* classNsPtr = EnsureNamespaceFromString(interp, cls, nsPtr);
    ...
    for (uint32_t i = 0; i < numNsBodies; ++i)
    {
        ...
        Tcl_Namespace* bodyNsPtr = EnsureNamespaceFromString(interp, (nsL ? nsName : NULL), nsPtr);
